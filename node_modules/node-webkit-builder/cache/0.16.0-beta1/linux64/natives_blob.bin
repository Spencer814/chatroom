...
var aY=new s();
var aZ=aX[0];
for(var aj=1;aj<aX.length;aj+=4){
var aA=aX[aj];
var aB=aX[aj+1];
var ba=aX[aj+2];
var bb=aX[aj+3];
var aC=(typeof(aB)==='number')&&bb<0?(-1-bb):
%FunctionGetPositionForOffset(ba,bb);
aZ--;
aY.push(new CallSite(aA,aB,aC,(aZ<0)));
}
return aY;
}
var bc=false;
function FormatStackTrace(Q,aX){
var aY=GetStackFrames(aX);
if((%IsFunction(G.prepareStackTrace))&&
!bc){
var bd=[];
%MoveArrayContents(aY,bd);
bc=true;
var be=(void 0);
try{
be=G.prepareStackTrace(Q,bd);
}catch(e){
throw e;
}finally{
bc=false;
...
var be=(bk[x]);
if((be===(void 0))){
bk=%object_get_prototype_of(bk);
continue;
}
bj=FormatStackTrace(bk,be);
(bk[x]=(void 0));
(bk[n]=bj);
}
return bj;
...
var bn=function(){};
function SetUpError(bo){
%FunctionSetInstanceClassName(bo,'Error');
var R=bo.name;
var bp=new o();
if(R!=='Error'){
%InternalSetPrototype(bo,G);
%InternalSetPrototype(bp,G.prototype);
}
%FunctionSetPrototype(bo,bp);
%AddNamedProperty(bo.prototype,'name',R,2);
%AddNamedProperty(bo.prototype,'message','',2);
%AddNamedProperty(
bo.prototype,'constructor',bo,2);
%SetCode(bo,function(bq){
if((new.target===(void 0)))return new bo(bq);
try{bn(this,bo);}catch(e){}
if(!(bq===(void 0))){
%AddNamedProperty(this,'message',(%_ToString(bq)),2);
}
});
%SetNativeFlag(bo);
return bo;
...
var br=MakeRangeError(176);
b.InstallGetterSetter(br,'stack',
bi,bl)
bn=function captureStackTrace(Q,bs){
%object_define_property(Q,'stack',{get:bi,
set:bl,
configurable:true});
%CollectStackTrace(Q,bs?bs:bn);
};
G.captureStackTrace=bn;
%InstallToContext([
...
"stack_overflow_boilerplate",br,
]);
b.Export(function(bt){
bt.ErrorToString=ErrorToString;
bt.MakeError=MakeError;
bt.MakeRangeError=MakeRangeError;
bt.MakeSyntaxError=MakeSyntaxError;
bt.MakeTypeError=MakeTypeError;
bt.MakeURIError=MakeURIError;
});
});
